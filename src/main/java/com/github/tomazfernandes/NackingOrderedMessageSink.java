 ...
 * @since 0.1.0
 */
public class NackingOrderedMessageSink<T> extends AbstractMessageProcessingPipelineSink<T> {
    ...
    protected CompletableFuture<Void> doEmit(Collection<Message<T>> messages, MessageProcessingContext<T> context) {
        ...
        CompletableFuture<Void> execution = messages.stream().reduce(CompletableFuture.completedFuture(null),
                (resultFuture, msg) -> CompletableFutures.handleCompose(resultFuture, (v, t) -> {
                    ...
                    return maybeNack(context, msg).thenCompose(theVoid -> CompletableFutures.failedFuture(t));
                }), (a, b) -> a);
        return execution.exceptionally(t -> null);
    }

    private CompletableFuture<Void> maybeNack(MessageProcessingContext<T> context, Message<T> msg) {
        return context.getAcknowledgmentCallback() instanceof NackAwareAcknowledgementCallback
                ? ((NackAwareAcknowledgementCallback<T>) context.getAcknowledgmentCallback()).onNack(msg)
